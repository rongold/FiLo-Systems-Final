...
$category = $colour = $date = $photo = $place= $description = $image = $destinationfile = "";
$category_err = $colour_err = $date_err = $photo_err = $place_err = $description_err = $image = "";
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
  ...
  if(empty($category_err) && empty($colour_err) && empty($date_err) && empty($description_err) && empty($place_err) && empty($image_err) ){
    ...
    $id=$_POST["item"];
    // update statement based on the form
    $sql = "UPDATE items SET category = '$category', colour = '$colour' ,
            date= '$date' , place = '$place' , description = '$description', photo = '$location' WHERE itemID = $id ";

    if(mysqli_query($link, $sql)){
      //if successful returns to admin home page
...
<body>
  ...
  <div class="container">
    <h2 class="text-center text-dark"> Edit Item <?php echo $_GET['id'] ?> </h2>
  </div>
  <div class="wrapper">
    <h2>Edit an Item</h2>
    ...
    <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" enctype="multipart/form-data">
      ...
      <input type="radio" name="category" <?php if (isset($category) && $category=="jewellery") echo "checked";?> value="jewellery">Other
